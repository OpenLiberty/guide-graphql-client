...
The name of each resolver is the method name, but it can be overridden with the [hotspot=querySystemTag hotspot=querySystemLoadTag file=0]`@Query` or [hotspot=mutationTag file=0]`@Mutation` annotations. In this case, the name of method [hotspot=systemInfo file=0]`getSystemInfo` is overridden as [hotspot=querySystemTag file=0]`system`, the name of method [hotspot=systemLoad file=0]`getSystemLoad` is overridden as [hotspot=querySystemLoadTag file=0]`systemLoad`, and the name of method [hotspot=editNote file=0]`setNote` is overridden as [hotspot=mutationTag file=0]`editNote`. The GraphQL request that goes over the wire will use the name overridden by the `@Query` and `@Mutation` annotation. Similarly with the [hotspot=systemInfo hotspot=systemLoad hotspot=editNote file=0]`@Name` annotation on the function inputs.

For example, the [hotspot=systemInfo file=0]`getSystemInfo()` function maps to the `system` resolver. The resolver returns a `SystemInfo` object, which is described by the `SystemInfo` class. Thus, the [hotspot=systemInfo file=0]`getSystemInfo()` function returns the type `SystemInfo`.
