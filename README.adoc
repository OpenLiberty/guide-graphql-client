...
This interface declares [hotspot=SystemInfo file=0]`querySystem()`, [hotspot=List<SystemLoad> file=0]`querySystemLoad()`, and [hotspot=Response file=0]`editNote()` methods for accessing each of the endpoints that are set up to access the `query` microservice.
