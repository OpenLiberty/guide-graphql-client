...
You will start with the GraphQL service created as part of the https://openliberty.io/guides/microprofile-graphql.html[Optimizing REST queries for microservices with GraphQL^] guide.
...
The typesafe client interface contains a function for each resolver available in the GraphQL service.
The JSON objects returned by the GraphQL service are converted to Java objects.
...
Use the [hotspot=clientBuilder file=1]`GraphQLClientBuilder`] class to create a client object that implements the `GraphQlClient` interface and can interact with the GraphQL service.
...
The [hotspot=graphQLClientDependency file=2]`smallrye-graphql-client` dependency provides the classes that use to interact with a GraphQL service.
...
The [hotspot=url file=3]`graphql.server` variable is defined in the [hotspot file=1]`server.xml` file.
...
It will create the two `system` services, a GraphQL service, and a `query` service that will interact with the GraphQL service.
