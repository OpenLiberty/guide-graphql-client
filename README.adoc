...
This interface declares [hotspot=clientUsed1 file=0]`querySystem()`, [hotspot=clientUsed2 file=0]`querySystemLoad()`, and [hotspot=clientUsed3 file=0]`editNote()` methods for accessing each of the endpoints that are set up to access the `query` microservice.
