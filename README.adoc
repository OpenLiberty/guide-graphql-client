...
When the `hostname` is specified to `system-java8`, but you can try out the operations using the hostname `system-java11` as well. 
...
```bash
curl -X 'POST' 'http://localhost:9084/query/mutation/system/note' -H 'Content-Type: application/json' -d '{"hostname": "system-java8","text": "I am trying out GraphQL on Open Liberty!"}'
```
You will recieve a `200` response code if the request is processed succesfully. 
