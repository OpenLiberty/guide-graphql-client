...
to create a `client` microservice that will make requests to the GraphQL microservice.
The GraphQL microservice retrieves data from multiple `system` microservices. 
The `client` makes requests to the GraphQL microservice, which then makes requests to the `system` microservice.
...
A typesafe client interface will be created.
...
[hotspot=clientApi file=0]`@GraphQlClient` annotation.
...
The [hotspot file=1]`ClientResource` class uses the [hotspot file=0]`GraphQlClient` interface
...
The `GraphQlClient` interface is used in a [hotspot=clientBuilder file=1]`GraphQLClientBuilder`
to create an object that implements the interface and can interact with the GraphQL service.
It will make requests to the URL specified by the [hotspot=url file=3]`graphql.server` variable
...
To use GraphQL client, the SmallRye GraphQL client dependencies and features need to be included.
...
The [hotspot=graphQLClientDependency file=2]`smallrye-graphql-client` dependency 
...
The `mvn package` command packages the `system`, `graphql`, and `client` services. 
...
You can also make requests to add a note to a system service at the `POST /client/properties/note` endpoint.
