...
The [hotspot file=1]`QueryResource` class uses the `GraphQlClient` interface to make requests to the `graphql` microservice and display the results. In a real application, you would be making requests to an external GraphQL API, or you would be doing further manipulation of the data after retrieval.
...
The [hotspot=graphQLClientDependency file=2]`smallrye-graphql-client` dependencies provide the classes that use to interact with a `graphql` microservice.
...
[role='command']
```
mkdir src\test\java\it\io\openliberty\guides\query
...
[role='command']
```
mkdir -p src/test/java/it/io/openliberty/guides/query
mkdir src/test/resources
```
--

Create a RESTful client interface for the `query` microservice.
...
This interface declares [hotspot=SystemInfo file=4]`querySystem()`, [hotspot=List<SystemLoad> file=4]`querySystemLoad()`, and [hotspot=Response file=4]`editNote()` methods for accessing each of the endpoints that are set up to access the `query`` microservice.

Create the test container class that access the `query` docker image that you built in previous section.
...
The [hotspot=createRestClient file=5]`createRestClient()` method creates a REST client instance with the QueryResourceClient interface. The [hotspot=getBaseURL file=5]`getBaseURL()` method constructs the URL that can access the `query` docker image.
...
Define the [hotspot=systemContainer file=6]`systemContainer` test container to start up the `system-java8` docker image, the [hotspot=graphqlContainer file=6]`graphqlContainer` test container to start up the `graphql` docker image, and the [hotspot=libertyContainer file=6]`libertyContainer` test container to start up the `query` docker image. Make sure that the containers use the same network.

The [hotspot=testEditNote file=6]`testEditNote()` verifies the mutation operation at the `/query/mutation/system/note` endpoint.

The [hotspot=testGetSystemLoad file=6]`testGetSystemLoad()` verifies the `/query/systemLoad/{hostnames}` endpoint with `hostnames` specified to `system-java8`.

The [hotspot=testGetSystem file=6]`testGetSystem()` verifies the `/query/system/{hostname}` endpoint with `hostnames` specified to `system-java8`.
...
`query/src/test/resources/log4j.properties`
----
...
[source, java, linenums, role='code_column hide_tags=copyright']
----
include::finish/query/src/test/resources/log4j.properties[]
----

The required [hotspot=testDependenies file=2]`dependencies` had already been added to the [hotspot file=2]`pom.xml` Maven configuration file for you, including JUnit5, JBoss RESTEasy client, Glassfish JSON, Testcontainers, and Log4J libraries.

To enable running the integration test by the Maven `verify` goal, the [hotspot=failsafe file=5]`maven-failsafe-plugin` plugin is also required.
