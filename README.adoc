...
For example, the [hotspot=systemInfo file=0]`getSystemInfo()` function maps to the `system` resolver. The resolver returns a `SystemInfo` object, which is described by the `SystemInfo` class. Thus, the [hotspot=systemInfo file=0]`system()` function returns the type `SystemInfo`.
