...
GraphQL is an open source data query language. You can use a GraphQL service to obtain data from multiple sources, such as APIs, databases, and other services. GraphQL services require less data fetching than REST services, which results in faster application load times and lower data transfer costs. This guide assumes you have a basic understanding of https://openliberty.io/docs/latest/microprofile-graphql.html[GraphQL concepts^]. If you're new to GraphQL, you might want to start with the https://openliberty.io/guides/microprofile-graphql.html[Optimizing REST queries for microservices with GraphQL^] guide first.
...
The [hotspot=editNote file=0]`editNote` `mutation` operation has the [hotspot=mutationTag file=0]`@Mutation` annotation on it. A `mutation` operation allows you to modify data, in this case, it allows you to add and edit a note to the system service. If the [hotspot=mutationTag file=0]`@Mutation` annotation were not placed on the function, it would be treated as if it mapped to a `query` operation. 
...
The [hotspot file=1]`QueryResource` class uses the [hotspot=clientBuilder file=1]`GraphQlClient` interface to make requests to the `graphql` microservice and display the results. In a real application, you would make requests to an external GraphQL API, and you might do further manipulation of the data after retrieval.
...
From the OpenAPI UI, test the read operation at the `GET /query/system/{hostname}` endpoint. This request retrieves the system properties for the `hostname` specified. The response is similar to the following example when the `hostname` is specified to `system-java8`, but you can try out the operations using the hostname `system-java11` as well:
...
The [hotspot=testGetSystem file=2]`testGetSystem()` verifies the `/query/system/{hostname}` endpoint with `hostname` specified as `system-java8`.

The [hotspot=testGetSystemLoad file=2]`testGetSystemLoad()` verifies the `/query/systemLoad/{hostnames}` endpoint with `hostnames` specified as `system-java8`.
